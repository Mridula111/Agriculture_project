        ...
        <main className="flex-1 p-6 lg:p-10 w-full max-w-[1600px] mx-auto space-y-6">
          <header className="pb-8 border-b border-stone-800">
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-white tracking-tight">
              Canopy Weather & Evapotranspiration
            </h1>
            <p className="text-stone-400 text-sm mt-1">
              Field radar metrics, precipitation deficits, and cane growth thermal units (GDD).
            </p>
          </header>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              ...
              <Card key={i} className="bg-stone-900 border-stone-800">
                <CardContent className="p-5 flex items-center justify-between">
                  <div>
                    <p className="text-xs uppercase font-bold text-stone-400 tracking-wider">{item.label}</p>
                    <p className="text-2xl font-black text-white mt-1">{item.value}</p>
                    <p className="text-xs text-amber-400 mt-1">{item.sub}</p>
                  </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card className="bg-stone-900 border-stone-800">
              <CardHeader>
                <CardTitle className="text-stone-200">5-Day Precipitation & Irrigation Outlook</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                {[
                  ...
                  <div key={idx} className="flex items-center justify-between p-3.5 rounded-xl bg-stone-950 border border-stone-800 text-sm">
                    <span className="font-semibold text-white">{row.day}</span>
                    <span className="text-amber-400 font-mono text-xs">{row.rain}</span>
                    <span className="text-stone-400 text-xs text-right">{row.advice}</span>
                  </div>
                ))}
              </CardContent>
            </Card>
            ...
            <Card className="bg-stone-900 border-stone-800">
              <CardHeader>
                <CardTitle className="text-stone-200">Growing Degree Days (GDD) Metric</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="p-4 rounded-xl bg-stone-950 border border-stone-800">
                  <p className="text-xs text-stone-400 uppercase font-bold tracking-wider">Accumulated Heat Units</p>
                  <p className="text-3xl font-black text-amber-400 mt-1">1,842 GDD</p>
                  <p className="text-xs text-stone-400 mt-1">Target for current tillering batch: 2,100 GDD</p>
                </div>
                <div className="p-4 rounded-xl bg-stone-950 border border-stone-800 text-xs text-stone-400 leading-relaxed">
